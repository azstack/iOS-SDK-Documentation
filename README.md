...
https://www.dropbox.com/s/31w3eihqtyd5zxa/AzStack_iOS_SDK.zip?dl=0
...
This delegate is called by AZStack SDK after receive [none] response from AZStack server, this function need to send azStackUserID, none to your seerver in order to get 1 identityToken (Identity Token).

At your server, identityToken must be generated by encrypt this string:
```objective-c
...
After your client received the identityToken from your server, you need to send it to AZStack server to finalize the authorzation by calling method:
